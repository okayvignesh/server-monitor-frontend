...
const MainMenu = () => {
	...
	const calculateTotalStorage = (storage: any) => {
		if (!Array.isArray(storage)) return 0;

		const validPartitions = storage.filter((s) => s.rw !== false);

		const totalBytes = validPartitions.reduce((acc, partition) => {
			return acc + (partition.size || 0);
		}, 0);

		const totalGB = totalBytes / 1024 ** 3;
		setTotalStorage(totalGB.toFixed(2));
	};
